    ...
    <x-slot name="header" :recipient="$recipient">
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        <button type="button" onclick="location.href='{{ route('admin.recipients.show', ['recipient' => $recipient->id]) }}'" class=" bg-gray-200 border-0 py-2 px-8 mr-8 focus:outline-none hover:bg-gray-400 rounded text-lg">戻る</button>
          所得計算：{{ $recipient->obligor->name }}
      </h2>
    </x-slot>

    <div class="py-8">
      <x-auth-validation-errors class="mb-4" :errors="$errors" />
      <form method="POST" action="{{ route('admin.recipients.obligors.calculations.update', ['recipient' => $recipient->id, 'obligor' => $recipient->obligor->id, 'calculation' => $recipient->obligor->calculation->id]) }}">
        @csrf
        @method('put')
                      ...
                      <div class="-m-2">
                        <div class="p-2 mx-auto">
                          <div class="relative">
                            ...
                            <input onkeyup="deducted_income()" type="number" id="income" name="income" value="{{ old('income', $recipient->obligor->calculation->income->income) }}" required placeholder="円 ※10万円控除前の金額を入力して下さい" class="w-full bg-gray-100 bg-opacity-50 rounded focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                          </div>
                        </div>
                        <div class="p-2 mx-auto">
                          <div class="relative">
                            ...
                            <select id="type" name="type" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                              @foreach ($income_type_categories as $income_type_category)
                                <option value="{{ $income_type_category->value }}" @if($income_type_category->value === $recipient->obligor->calculation->income->type) selected @endif>
                      ...
                      <div class="-m-2">
                        <div class="p-2 mx-auto">
                          <div class="relative">
                            ...
                            <input type="number" id="disabled" name="disabled" value="{{ old('disabled', $recipient->obligor->calculation->deduction->disabled) }}" required placeholder="人" class="w-full bg-gray-100 bg-opacity-50 rounded focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                          </div>
                        </div>
                        <div class="p-2 mx-auto">
                          <div class="relative">
                            <label for="specially_disabled" class="leading-7 text-sm text-gray-600">特別障がい者控除</label>
                            <input type="number" id="specially_disabled" name="specially_disabled" value="{{ old('specially_disabled', $recipient->obligor->calculation->deduction->specially_disabled) }}" required placeholder="人" class="w-full bg-gray-100 bg-opacity-50 rounded focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                          </div>
                        </div>
                        <div class="p-2 mx-auto">
                          <div class="relative">
                            <label for="singleparent_or_workingstudent" class="leading-7 text-sm text-gray-600">ひとり親または勤労学生控除</label>
                            <input type="number" id="singleparent_or_workingstudent" name="singleparent_or_workingstudent" value="{{ old('singleparent_or_workingstudent', $recipient->obligor->calculation->deduction->singleparent_or_workingstudent) }}" required placeholder="人" class="w-full bg-gray-100 bg-opacity-50 rounded focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                          </div>
                        </div>
                        <div class="p-2 mx-auto">
                          <div class="relative">
                            <label for="special_spouse" class="leading-7 text-sm text-gray-600">配偶者特別控除</label>
                            <input type="number" id="special_spouse" name="special_spouse" value="{{ old('special_spouse', $recipient->obligor->calculation->deduction->special_spouse) }}" required placeholder="円" class="w-full bg-gray-100 bg-opacity-50 rounded focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                          </div>
                        </div>
                        <div class="p-2 mx-auto">
                          <div class="relative">
                            <label for="medical_expense" class="leading-7 text-sm text-gray-600">医療費控除</label>
                            <input type="number" id="medical_expense" name="medical_expense" value="{{ old('medical_expense', $recipient->obligor->calculation->deduction->medical_expense) }}" required placeholder="円" class="w-full bg-gray-100 bg-opacity-50 rounded focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                          </div>
                        </div>
                        <div class="p-2 mx-auto">
                          <div class="relative">
                            <label for="small_enterprise" class="leading-7 text-sm text-gray-600">小規模企業共済</label>
                            <input type="number" id="small_enterprise" name="small_enterprise" value="{{ old('small_enterprise', $recipient->obligor->calculation->deduction->small_enterprise) }}" required placeholder="円" class="w-full bg-gray-100 bg-opacity-50 rounded focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                          </div>
                        </div>
                        <div class="p-2 mx-auto">
                          <div class="relative">
                            <label for="other" class="leading-7 text-sm text-gray-600">その他控除</label>
                            <input type="number" id="other" name="other" value="{{ old('other', $recipient->obligor->calculation->deduction->other) }}" required placeholder="円" class="w-full bg-gray-100 bg-opacity-50 rounded focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                          </div>
                        </div>
                        <div class="p-2 mx-auto">
                          <div class="relative">
